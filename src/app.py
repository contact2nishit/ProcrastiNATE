    ...
    try:
        async with app.state.pool.acquire() as conn:
            ...
            if user1:
                raise HTTPException(
                    status_code=409,
                    detail="Already registered with same username or email",
                )
            ...
            await conn.execute(
                "INSERT INTO achievements(user_id, levels) VALUES($1, $2)", user_id, 1
            )
...
async def login(
    ...
    try:
        user = await authenticate_user(
            form_data.username, form_data.password, app.state.pool
        )
        if not user:
            raise HTTPException(
    ...
    async with app.state.pool.acquire() as conn:
        ...
        if user_row:
            ...
            if user_row:
                ...
                await conn.execute(
                    "INSERT INTO achievements(user_id, levels) VALUES($1, $2)",
                    user_id,
                    1,
                )
        # Ensure all users have an achievements record (for existing users who may not have one)
        achievements_exists = await conn.fetchval(
            "SELECT 1 FROM achievements WHERE user_id = $1", user_id
        )
...
async def sync(
    token: Annotated[str, Depends(oauth2_scheme)], status_code=status.HTTP_201_CREATED
    ...
    try:
        ...
        async with app.state.pool.acquire() as conn:
            ...
            if not row or not row["google_access_token"]:
                raise HTTPException(
                    status_code=400, detail="User has not linked Google account."
                )
        ...
        async with app.state.pool.acquire() as conn:
            ...
            for assignment in chosen_schedule.assignments:
                ...
                assign_id = await conn.fetchval(
                    "INSERT INTO assignments(user_id, assignment_name, effort, deadline) VALUES($1, $2, $3, $4) RETURNING assignment_id",
                    user.user_id,
                    assignment.name,
                    assignment.effort,
                    assignment.due,
                ...
                for timeslot in assignment.schedule.slots:
                    ...
                    occurence_id = await conn.fetchval(
                        "INSERT INTO assignment_occurences(user_id, assignment_id, start_time, end_time, xp_potential) VALUES($1, $2, $3, $4, $5) RETURNING occurence_id",
                        user.user_id,
                        assign_id,
                        timeslot.start,
                        timeslot.end,
                        timeslot.xp_potential,
                    )
            ...
            for chore in chosen_schedule.chores:
                ...
                assign_id = await conn.fetchval(
                    "INSERT INTO chores(chore_name, effort, start_window, end_window, user_id) VALUES($1, $2, $3, $4, $5) RETURNING chore_id",
                    chore.name,
                    chore.effort,
                    chore.window[0],
                    chore.window[1],
                ...
                for timeslot in chore.schedule.slots:
                    ...
                    timeslot.end = enforce_timestamp_utc(timeslot.end)
                    occurence_id = await conn.fetchval(
                        "INSERT INTO chore_occurences(chore_id, start_time, end_time, user_id, xp_potential) VALUES($1, $2, $3, $4, $5) RETURNING occurence_id",
                        assign_id,
                        timeslot.start,
                        timeslot.end,
                        user.user_id,
...
@app.post("/update")
async def update(
    changes: UpdateRequestDataModel, token: Annotated[str, Depends(oauth2_scheme)]
) -> UpdateResponseDataModel:
    """
            ...
            if changes.future_occurences:
                ...
                if changes.new_loc_or_link:
                    await conn.execute(
                        ...
                        changes.new_loc_or_link,
                        changes.meeting_id,
                        user.user_id,
                    )
                    updates.append("location/link")
                return UpdateResponseDataModel(
                    clashed=False,
                    message=f"Updated {', '.join(updates)} for all future occurrences.",
                )
            else:
                ...
                return UpdateResponseDataModel(
                    clashed=False, message="Occurrence updated successfully."
                )
    except HTTPException as e:
        raise e
    except Exception as e:
...
@app.post("/delete")
async def delete(
    deletion: DeleteRequestDataModel, token: Annotated[str, Depends(oauth2_scheme)]
) -> MessageResponseDataModel:
    """
        ...
        async with app.state.pool.acquire() as conn:
            ...
            if deletion.event_type == "meeting":
                if deletion.remove_all_future:
                    ...
                    return MessageResponseDataModel(
                        message="Deleted this and all future occurrences."
                    )
                else:
                ...
                return MessageResponseDataModel(
                    message="Assignment and all its occurrences deleted."
                )
                ...
                return MessageResponseDataModel(
                    message="Chore and all its occurrences deleted."
                )
            else:
...
async def reschedule(
    re: RescheduleRequestDataModel, token: Annotated[str, Depends(oauth2_scheme)]
) -> ScheduleResponseFormat:
    """
    ...
    try:
        async with app.state.pool.acquire() as conn:
            ...
            occ_table = (
                "assignment_occurences"
                if re.event_type == "assignment"
                else "chore_occurences"
            )
            id_col = "assignment_id" if re.event_type == "assignment" else "chore_id"
            name_col = (
                "assignment_name" if re.event_type == "assignment" else "chore_name"
            )
            ...
            if not row:
                raise HTTPException(
                    status_code=404, detail=f"{re.event_type.capitalize()} not found"
                )
            ...
            future_occs = [
                o["occurence_id"] for o in old_occs if o["end_time"] >= now_utc
            ]
            if future_occs:
                await conn.execute(
            ...
            if re.event_type == "assignment":
                ...
                if re.new_window_end is not None:
                    ...
                    if isinstance(re.new_window_end, int):
                        raise HTTPException(
                            status_code=400,
                            detail="Deadline must be a datetime, not an integer",
                        )
                ...
                new_due = (
                    re.new_window_end if re.new_window_end is not None else old_due
                )
                assignment_req = AssignmentInRequest(
                    name=name, effort=new_effort, due=new_due
                )
            else:
                update_fields = []
                update_values = []
                ...
                if re.new_window_start is not None:
                    ...
                    if isinstance(re.new_window_start, int):
                        raise HTTPException(
                            status_code=400,
                            detail="start_window must be a datetime, not an integer",
                        )
                ...
                if re.new_window_end is not None:
                    ...
                    if isinstance(re.new_window_end, int):
                        raise HTTPException(
                            status_code=400,
                            detail="end_window must be a datetime, not an integer",
                        )
                    update_values.append(re.new_window_end)
                    param_idx += 1
                if update_fields:
            ...
            assignments = await conn.fetch(
                "SELECT start_time, end_time FROM assignment_occurences WHERE user_id = $1"
                + (
                    f" AND assignment_id != {obj_id}"
                    if re.event_type == "assignment"
                    else ""
                ),
                user.user_id,
            ...
            chores = await conn.fetch(
                "SELECT start_time, end_time FROM chore_occurences WHERE user_id = $1"
                + (f" AND chore_id != {obj_id}" if re.event_type == "chore" else ""),
                user.user_id,
            )
            blocked_times = []
            for r in meetings:
    ...
    try:
        async with app.state.pool.acquire() as conn:
            ...
            if len(meetings) > 0:
                ...
                for occurence_list in partitioned_meetings:
                    ...
                    for occurence in occurence_list:
                        meeting_start_end_times += [
                            [occurence["start_time"], occurence["end_time"]]
                        ]
            ...
            if len(assignments) > 0:
                partitioned_assignments = partition_by_meeting_id(
                ...
                for occurence_list in partitioned_assignments:
                    ...
                    for occurence in occurence_list:
                        ass_start_end_times += [
                            TimeSlot(
                                start=occurence["start_time"], end=occurence["end_time"]
                            )
                        ]
                        effort_assigned += (
                            occurence["end_time"] - occurence["start_time"]
                        ).total_seconds() // 60
                    ...
                    assignment_response = AssignmentInResponse(
                        ...
                        schedule=ScheduledTaskInfo(
                            ...
                            status=(
                                "unschedulable"
                                if effort_assigned == 0
                                else (
                                    "partially_scheduled"
                                    if effort_assigned < occurence["effort"]
                                    else "fully_scheduled"
                                )
                            ),
            ...
            if len(chores) > 0:
                ...
                for occurence_list in partitioned_chores:
                    ...
                    chore_response = ChoreInResponse(
                        ...
                        effort=occurence_list[0]["effort"],
                        window=[
                            occurence_list[0]["start_window"],
                            occurence_list[0]["end_window"],
                        ],
                        schedule=ScheduledTaskInfo(
                            effort_assigned=effort_assigned,
                            status=(
