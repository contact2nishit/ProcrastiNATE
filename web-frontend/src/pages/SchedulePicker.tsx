...
const SchedulePicker = () => {
    ...
    return (
        <div className="flex-1 bg-black p-2.5 min-h-screen">
            <h1 data-testid="schedule-picker-title" className="text-3xl font-bold text-white text-center my-5">Pick a Schedule</h1>

            {conflicting_meetings.length > 0 && (
                <div data-testid="conflicting-meetings-warning" className="bg-red-100 rounded-lg p-2.5 mb-2.5">
                    <h3 className="text-red-800 font-bold text-base">Conflicting Meetings:</h3>
                    {conflicting_meetings.map((m: string, i: number) => (
                        <p key={i} data-testid={`conflicting-meeting-${i}`} className="text-red-800 text-sm">{m}</p>
                    ))}
                </div>
            )}
                ...
                <div data-testid="schedule-modal" className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
                    <div className="bg-white rounded-lg p-5 w-[90%] max-w-2xl max-h-[80%] overflow-y-auto">
                        {selectedScheduleIdx !== null && schedules[selectedScheduleIdx] && (
                            <>
                                <h2 data-testid="modal-schedule-title" className="text-2xl font-bold mb-2.5 text-black">Schedule #{selectedScheduleIdx + 1}</h2>
                                
                                <h3 data-testid="assignments-section" className="text-lg font-bold mt-4 text-gray-800">Assignments</h3>
                                {schedules[selectedScheduleIdx]?.assignments.length === 0 && (
                                    <p data-testid="assignments-none" className="text-gray-500 italic ml-2">None</p>
                                )}
                                {schedules[selectedScheduleIdx]?.assignments.map((a: any, i: number) => (
                                    <div key={i} data-testid={`assignment-item-${i}`} className="mb-2.5 p-2 bg-gray-100 rounded-md">
                                ...
                                <h3 data-testid="chores-section" className="text-lg font-bold mt-4 text-gray-800">Chores</h3>
                                {schedules[selectedScheduleIdx]?.chores.length === 0 && (
                                    <p data-testid="chores-none" className="text-gray-500 italic ml-2">None</p>
                                )}
                                {schedules[selectedScheduleIdx]?.chores.map((c: any, i: number) => (
                                    <div key={i} data-testid={`chore-item-${i}`} className="mb-2.5 p-2 bg-gray-100 rounded-md">
                                ...
                                <h3 data-testid="conflicts-section" className="text-lg font-bold mt-4 text-gray-800">Conflicts</h3>
                                {schedules[selectedScheduleIdx]?.conflicting_assignments.length === 0 &&
                                 ...
                                 schedules[selectedScheduleIdx]?.not_enough_time_chores.length === 0 && (
                                    <p data-testid="conflicts-none" className="text-gray-500 italic ml-2">None</p>
                                )}
                                {schedules[selectedScheduleIdx]?.conflicting_assignments.map((n: string, i: number) => (
                                    <p key={i} data-testid={`conflict-assignment-${i}`} className="text-red-800 text-sm">Assignment conflict: {n}</p>
                                ))}
                                {schedules[selectedScheduleIdx]?.conflicting_chores.map((n: string, i: number) => (
                                    <p key={i} data-testid={`conflict-chore-${i}`} className="text-red-800 text-sm">Chore conflict: {n}</p>
                                ))}
                                {schedules[selectedScheduleIdx]?.not_enough_time_assignments.map((n: string, i: number) => (
                                    <p key={i} data-testid={`not-enough-time-assignment-${i}`} className="text-red-800 text-sm">Not enough time for assignment: {n}</p>
                                ))}
                                {schedules[selectedScheduleIdx]?.not_enough_time_chores.map((n: string, i: number) => (
                                    <p key={i} data-testid={`not-enough-time-chore-${i}`} className="text-red-800 text-sm">Not enough time for chore: {n}</p>
                                ))}

                                <h3 data-testid="meetings-section" className="text-lg font-bold mt-4 text-gray-800">Meetings</h3>
                                {meetings.length === 0 && (
                                    <p data-testid="meetings-none" className="text-gray-500 italic ml-2">None</p>
                                )}
                                {meetings.map((m: { name: string; start_end_times: [string, string][] }, i: number) => (
                                    <div key={i} data-testid={`meeting-item-${i}`} className="mb-2.5 p-2 bg-gray-100 rounded-md">
                                ...
                                <p data-testid="potential-xp-display" className="font-bold text-base text-gray-800">Potential XP: {schedules[selectedScheduleIdx]?.total_potential_xp}</p>
